...
public class HashSet<E> extends HashTable<E> {
   ...
   * @note Time Complexity: O(n*m) where n is the size of the collection and m is the size of the
   *     table
   * @implNote
   */
  ...
  public static void main(String[] args) {
    ...
    // Test insert
    System.out.println(
        "Duplicate insert (should be false): " + hashSet.insert("One")); // Expected: false
    ...
    System.out.println(
        "Contains 'Three' after removal: " + hashSet.search("Three")); // Expected: false

    // Test addAll
    String[] toAdd = {"Four", "Five"};
    hashSet.addAll(toAdd);
    System.out.println(
        "After addAll, contains 'Four': " + hashSet.search("Four")); // Expected: true

    // Test containsAll
    System.out.println(
        "Contains all ['Four', 'Five']: " + hashSet.containsAll(toAdd)); // Expected: true

    // Test toList
    System.out.println(
        "List representation: "
            + hashSet.toList()); // Expected: [null, null, ..., "Four", "Five", ...]
  }
}
